...
- **Controls Lens (1.1 as meta-objective):** Objective 1.1 isn't one objective — it's a lens applied to everything. Every configuration, design decision, and exercise action gets tagged with its control classification: category (Technical / Managerial / Operational / Physical) and type (Preventive / Detective / Corrective / Compensating / Deterrent / Directive). A firewall rule is a technical preventive control. Enabling logging is a technical detective control. Changing a default password is a technical corrective control. Writing a design doc is a managerial directive control. Segmenting VLANs because you can't physically air-gap is a compensating control. This tagging is not extra work — it's the persistent background filter through which all lab activity is interpreted. The exam tests this at the Analyzing tier: given a scenario, classify the control.
